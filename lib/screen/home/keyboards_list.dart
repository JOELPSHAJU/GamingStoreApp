...
import 'package:gaming_shop_app_ui/screen/detail/detail_screen_keyboard.dart';
import 'package:svg_flutter/svg.dart';

class KeyboardsList extends StatefulWidget {
  const KeyboardsList({super.key});

  @override
  State<KeyboardsList> createState() => _KeyboardsListState();
}

class _KeyboardsListState extends State<KeyboardsList> {
  ...
  GestureDetector gamingItems(int index) {
    ...
    if (currentSelectItems == index) {
      scale = 1.35;
      backgroundColor = keyboardselectedBackground;

      textColor = Colors.white;
    }
    final game = keyboardList[index];
    ...
    return GestureDetector(
      onTap: () {
        ...
        Navigator.push(
          context,
          MaterialPageRoute(
            builder: (context) => DetailScreenKeyboard(game: game),
          ),
        );
        if (currentSelectItems == index) {
        } else {
